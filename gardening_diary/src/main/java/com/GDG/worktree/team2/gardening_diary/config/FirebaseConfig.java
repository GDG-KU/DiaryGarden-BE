    ...
    public FirebaseApp firebaseApp() throws IOException {
        ...
        // 2. 환경 변수 또는 시스템 프로퍼티에서 Firebase 자격 증명 확인 (.env 파일에서 로드 가능)
            ...
            if (firebaseCredentials != null && !firebaseCredentials.isEmpty()) {
                ...
                // classpath: 접두사 제거
                String filePath = serviceAccountPath.replace("classpath:", "");
                ClassPathResource resource = new ClassPathResource(filePath);
                
                if (!resource.exists()) {
                    throw new IllegalStateException("Firebase 서비스 계정 키 파일을 찾을 수 없습니다: " + filePath);
                }
                
                serviceAccount = resource.getInputStream();
                System.out.println("Firebase credentials loaded from classpath: " + filePath);
            }
        }
